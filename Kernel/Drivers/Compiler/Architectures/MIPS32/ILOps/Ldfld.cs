            ...
            else
            {
                ...
                for (int i = memSize + (memSize % 2); i > 0; i -= 2)
                {
                    if (sizeToSub != sizeNotInMem)
                    {
                        ...
                        conversionState.Append(new ASMOps.Mov() { Size = ASMOps.OperandSize.Byte, Src = (offset + i - 2).ToString() + "($t2)", Dest = "$t1", SrcIsMemory = true });
                        conversionState.Append(new ASMOps.Push() { Size = ASMOps.OperandSize.Word, Src = "$t0" });
                    }
                    else
                    {
                        conversionState.Append(new ASMOps.Mov() { Size = ASMOps.OperandSize.Word, Src = (offset + i - 2).ToString() + "($t2)", Dest = "$t0", SrcIsMemory = true });
                        conversionState.Append(new ASMOps.Push() { Size = ASMOps.OperandSize.Word, Src = "$t0" });
                    }
                }
