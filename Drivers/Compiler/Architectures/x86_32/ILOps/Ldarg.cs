    ...
    public class Ldarg : IL.ILOps.Ldarg
    {
        ...
        public override void Convert(ILConversionState conversionState, ILOp theOp)
        {
            ...
            else
            {
                ...
                if (bytesForArg == 4)
                {
                    conversionState.Append(new ASMOps.Push() { Size = ASMOps.OperandSize.Dword, Src = "[EBP+" + BytesOffsetFromEBP.ToString() + "]" });
                }
                else if (bytesForArg == 8)
                {
                    conversionState.Append(new ASMOps.Push() { Size = ASMOps.OperandSize.Dword, Src = "[EBP+" + (BytesOffsetFromEBP + 4).ToString() + "]" });
                    conversionState.Append(new ASMOps.Push() { Size = ASMOps.OperandSize.Dword, Src = "[EBP+" + BytesOffsetFromEBP.ToString() + "]" });
                }
                else
                {
